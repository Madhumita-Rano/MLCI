subroutine prune(length,llast,dnorm,eval,i_got_hit,prune_all,ieig)
  ...
  !mr edit
   ...
   !write(*,*) "size ",lshift-1  !stop -start
  !mrcharac = char(mrprune)
  !i_try1 = i_try + 70
  !open(i_try1,file='reject'//trim(char(i_try))//'.dat',status='unknown')
  !write(i_try1,*)i_try
  !mr edit
  do ici = length - lshift + 1, length
     ...
     do n=1,nword
        !mr edit
        !write(*,*)icij(1,n,ici),icij(2,n,ici)
        !mr edit
        icij(1,n,ici) = 0.0
        icij(2,n,ici) = 0.0
     enddo
